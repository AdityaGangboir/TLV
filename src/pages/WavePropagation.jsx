...
const WavePropagation = () => {
  ...
  const vswr =
    Math.abs(gammaL) < 0.9999
      ? (1 + Math.abs(gammaL)) / (1 - Math.abs(gammaL))
      : 999;
  ...
  const generateWaveData = () => {
    ...
    for (let i = 0; i <= points; i++) {
      ...
      const forwardI =
        (inputs.V0 / Zc_real) * Math.cos(omega * time - beta * z - Math.PI / 2);
      const reflectedI =
        -(inputs.V0 / Zc_real) *
        gammaL *
        Math.cos(omega * time + beta * z - Math.PI / 2);
  ...
  const scaleY = (y) =>
    margin.top + plotHeight / 2 - (y / (inputs.V0 * 2)) * (plotHeight / 2);
      ...
      <div className="wave-prop-wrapper">
        ...
        <div className="wave-prop-header">
          ...
          <div>
            ...
            <p className="wave-prop-subtitle">
              Simulate and analyze electromagnetic wave propagation along
              transmission lines
            </p>
          </div>
        </div>
